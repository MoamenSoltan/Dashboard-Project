          ...
          <div className="mt-10">
            {
              links.map((item,index)=>(
                <div key={index}>
                  ...
                  {
                    item.links.map((link)=>(
                      <NavLink to={`/${link.name}` } key={link.name} style={({isActive})=>({backgroundColor:
                      isActive?currentColor:''})} onClick={()=>handleCloseSideBar()}className={({isActive})=>isActive?activeLink:normalLink }>
